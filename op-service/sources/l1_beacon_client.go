...
func (cl *L1BeaconClient) GetBlobs(ctx context.Context, ref eth.L1BlockRef, hashes []eth.IndexedBlobHash) ([]*eth.Blob, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w; failed to get blob sidecars for L1BlockRef %s: %w", errBeaconBlobs, ref, err)
	}
	blobs, err = blobsFromSidecars(blobSidecars, hashes)
	if err != nil {
		return nil, fmt.Errorf("%w; failed to get blobs from sidecars for L1BlockRef %s: %w", errBeaconBlobs, ref, err)
	}
	return blobs, nil
}
